...
$pdf->writeSubTitle('Registration Statistics From ' . $from_date->format("Y-m-d") . ' to ' . $to_date->format("Y-m-d"));
$pdf->Ln(3);

$pdf->SetWidths(array(30, 30, 30, 30));
$pdf->SetAligns(array('C', 'C', 'C'));
$pdf->Row(array('Date', 'OPD', 'EMR'), true);
...
for ($index = 0; $index <= $diff; $index++) {
    ...
    $sql_opd = "select count(PID) as 'count' 
                from patient  
                where who_department_id = 2 AND patient.CreateDate LIKE '". $date. "%'
                group by substring(patient.CreateDate,1,10)";
    $result_opd = $this->db->query($sql_opd);
    $row_opd = $result_opd->first_row();

    $sql_adm = "select count(PID) as 'count' 
                from patient  
                where who_department_id = 1 AND patient.CreateDate LIKE '". $date. "%'
                group by substring(patient.CreateDate,1,10)";
    $result_adm = $this->db->query($sql_adm);
    $row_adm = $result_adm->first_row();

//    $sql_null = "select count(PID) as 'count'
//                from patient
//                where who_department_id IS NULL AND patient.CreateDate LIKE '". $date. "%'
//                group by substring(patient.CreateDate,1,10)";
//    $result_null = $this->db->query($sql_null);
//    $row_null = $result_null->first_row();
...
//    $null_count = $row_null ? $row_null->count : 0;

    $pdf->Row(array($date, $opd_count, $adm_count));
    date_add($from_date, new DateInterval('P1D'));
}

$pdf->Output('Registration Statistic From ' . $from_date->format("Y-m-d") . ' To ' . $to_date->format("Y-m-d"). '.pdf', 'i');
?>