...
unset($result);
$sql = "SELECT * FROM patient_allergy where ( PID ='" . $pid . "' ) AND (Active=1)  ORDER BY Name";
$result = $this->db->query($sql);
...
$pdf->SetWidths(array(128));
    ...
    foreach ($visit_JSON as $row) {
        $count++;
        $notes= Modules::run('opd/get_nursing_notes',$row["OPDID"],'opd_visit/view/'.$row["OPDID"],"DATA");
        if ($count > 5) {
            break;
        }
        $pdf->SetWidths(array(24, 24, 40, 40));
