...
$pdf->writeSubTitle('Visit From ' . $from_date->format("Y-m-d") . ' to ' . $to_date->format("Y-m-d"));
$pdf->Ln(3);
...
for ($index = 0; $index <= $diff; $index++) {
    ...
    $sql_opd = "select count(PID) as 'count' from opd_visits as ov where ov.DateTimeOfVisit like '{$date}%' group by substring(ov.DateTimeOfVisit,1,10)";
    $result_opd = $this->db->query($sql_opd);
    $row_opd = $result_opd->first_row();

    $sql_adm = "select count(PID) as 'count' 
                from emergency_admission  
                where emergency_admission.DateTimeOfVisit like '". $date. "%'
                group by substring(emergency_admission.DateTimeOfVisit,1,10)";
...
$pdf->Output('Visit_from_ ' . $from_date->format("Y-m-d") . ' to ' . $to_date->format("Y-m-d"). '.pdf', 'i');
?>