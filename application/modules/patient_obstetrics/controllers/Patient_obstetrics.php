 ...
 */
class Patient_obstetrics extends FormController
{
    var $_department;
    ...
    function __construct()
    {
        ...
        $this->load->model('m_patient_obstetrics');
 ...
 /*   public function add($pid)
    {
        ...
        $this->DEPARTMENT='PA';
        if (!has_permission('patient_obstetrics', 'create')) {
            $this->show_no_permission();
        }
    ...
    public function add($pid, $ref_id)
    {
        ...
        $data['pid'] = $pid;
        $data['ref_id'] = $ref_id;
        $data['default_comefrom'] = 'HGM';
        $data['default_reason'] = '7';
        $data['default_name'] = '';
        $data['default_status'] = '';
        $data['default_active'] = '';
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->m_patient_obstetrics->insert($data);
            $this->session->set_flashdata(
                'msg', 'REC: Ficha Clinica Do Parto criada para ' . $pid
            );
    ...
    public function create($pid)
    {
        ...
        if ($this->form_validation->run($this) == FALSE) {
            ...
            if( $consulta =  $this->m_patient_active_list->insert($data_insert)) {
                ...
                $this->redirect_if_no_continue('patient_obstetrics');
    ...
    {
        if (!has_permission('patient_obstetrics', 'edit')) {
            $this->show_no_permission();
        }
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->render('form_edit_patient_obstetrics', $data);
        } else {
            $data_update = array(
                'EntryTime' => $this->input->post('entry_time'),
                'HospitalizationReason' => $this->input->post('reason'),
    ...
    public function search($department)
    {
        ...
        if (!has_permission('patient_obstetrics', 'view')) {
            $this->show_no_permission();
        }
        $qry = "SELECT
              ...
              /*  LEFT JOIN doctor ON doctor.Doctor_ID = patient_active_list.Doctor_ID*/
                WHERE patient_active_list.Active = 1 
                AND   patient_active_list.Service = 27 
        ...
        $page->setColOption('Status', array('stype' => 'select',
            'editoptions' => array(
                'value' => ':All;'.lang('Pending').':'.lang('Pending').';'.lang('Observe').':'.lang('Observe')
            ), 'width' => '70'));
...
//        registrar group
        if (Modules::run('permission/check_permission', 'patient_obstetrics', 'edit')) {
            $page->gridComplete_JS
                = "function() {
            $('#patient_list .jqgrow').mouseover(function(e) {
                var rowId = $(this).attr('id');
                ...
                if (status == 'Pending' || status == 'Pendente') {
                    window.location='" . site_url("/patient_obstetrics/edit") . "/'+rowId+'';
                } else {
                     window.location='" . site_url("patient_obstetrics/redirect_for_doctor") . "/'+rowId+'';
                }
            });
            }";
        }
...
//        }
    ...
    public function add_observe($pid, $active_list_id)
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->redirect_if_no_continue('patient_obstetrics/view/' . $pa_id);
        }
    }
    ...
    public function redirect_for_doctor($active_list_id)
    {
        ...
        switch ($active_list->Department) {
            ...
            case 'OPD':
                ...
                break;
            case 'PA':
                $this->redirect_if_no_continue('patient_obstetrics/view/' . $active_list->VisitID);
                break;
            default:
