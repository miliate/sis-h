 ...
 */
class Patient_operation extends FormController
{
    var $_department;
    ...
    public function get_dropdown_services($department_id = 56, $type = 'json')
    {
        ...
        $result = $this->m_hospital_service->order_by('abrev')->get_many_by(array('department_id' => 2));
    ...
    public function create($pid, $ref_id)
    {
        ...
        $this->DEPARTMENT='OP';
        if (!has_permission('patient_operation_order', 'create')) {
            $this->show_no_permission();
            //exit;
        }
        ...
        $data['default_reason'] = '';
        $data['default_destination'] = 'Cirurgia';
        ...
        if ($this->form_validation->run($this) == FALSE) {
            ...
            if( $consulta =  $this->m_patient_active_list->insert($data_insert)) {
                ...
                $this->redirect_if_no_continue('patient_operation');
    ...
    {
        if (!has_permission('operation', 'edit')) {
            $this->show_no_permission();
            exit;
        }
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->render('form_edit_patient_operation', $data);
        } else {
            $data_update = array(
                'EntryTime' => $this->input->post('entry_time'),
                'HospitalizationReason' => $this->input->post('reason'),
    ...
    public function search($department)
    {
        ...
        if (!has_permission('patient_operation_order', 'view')) {
            $this->show_no_permission();
        }
        $qry = "SELECT
...
//        registrar group
        if (Modules::run('permission/check_permission', 'operation', 'edit')) {
            $page->gridComplete_JS
                = "function() {
            $('#patient_list .jqgrow').mouseover(function(e) {
                var rowId = $(this).attr('id');
                ...
                if (status == 'Pending' || status == 'Pendente') {
                    window.location='" . site_url("/patient_operation/edit") . "/'+rowId+'';
                } else {
                     window.location='" . site_url("patient_operation/redirect_for_doctor") . "/'+rowId+'';
                }
            });
            }";
        }
...
//        }
    ...
    public function add_observe($pid, $active_list_id)
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->redirect_if_no_continue('patient_operation/view/' . $pa_id);
        }
    }
    ...
    public function redirect_for_doctor($active_list_id)
    {
        ...
        switch ($active_list->Department) {
            ...
            case 'OPD':
                ...
                break;
            case 'PA':
                $this->redirect_if_no_continue('patient_operation/view/' . $active_list->VisitID);
                break;
            default:
