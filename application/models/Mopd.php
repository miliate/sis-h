...
class Mopd extends My_Model
{
	...
	function get_info($opd_id){
        ...
        $opd_id = mysql_real_escape_string($opd_id);
		$dataset = array();
		$sql=" select opd_visits.*,visit_type.VTYPID as visit_type_id,visit_type.Name as visit_type_name,opd_visits.Doctor as doc_id,opd_visits.DateTimeOfVisit as DateTimeOfVisit, concat(user.Title,user.FirstName,' ',user.OtherName ) as Doctor ";
        $sql .= " FROM  opd_visits " ;
